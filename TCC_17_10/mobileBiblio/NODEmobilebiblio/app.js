...
const fetch = require("node-fetch"); // Para baixar o PDF
...
// --- Início: gerenciamento de pré-reservas (substituir as rotas antigas) ---

/**
 * POST /pre_reservas
 * Body: { usuario_id, livro_id }
 * Cria uma pré-reserva: status 'aguardando', data_retirada_max = NOW() + 24h. 
 ...
 * DELETE /pre_reservas
 * Body: { usuario_id, livro_id }
 * Cancela manualmente uma pré-reserva em 'aguardando': deleta a linha e incrementa estoque.
 */
...
/**
 * POST /pre_reservas/retirar
 * Body: { usuario_id, livro_id }
 * Marca a pré-reserva 'aguardando' como 'retirado' (usuario pegou o livro).
 * Não altera estoque (foi decrementado no momento da pré-reserva).
 */
...
/**
 * POST /pre_reservas/devolver
 * Body: { usuario_id, livro_id }
 * Marca a pré-reserva 'retirado' como 'devolvido' e incrementa estoque.
 */
...
/**
 * GET /pre_reservas/check?usuario_id=...&livro_id=...
 * Retorna o status mais recente e data_retirada_max.
 * Se a reserva estiver expirada (status='aguardando' e data_retirada_max < NOW()), 
 * a rota automaticamente deleta a pré-reserva e restaura o estoque (comportamento solicitado).
 */
...
/**
 * cleanupExpiredReservations - função que roda periodicamente no servidor para
 * apagar reservas expiradas (status = 'aguardando' && data_retirada_max < NOW())
 * e restaurar o estoque. Recomendo rodar a cada 5 minutos.
 */
...
const storage = multer.diskStorage({
  ...
  filename: function (req, file, cb) {
    ...
    // Usa timestamp para evitar conflitos até ter o ID
...
// IMPORTANTE: Adicione esta linha para servir arquivos estáticos
...
app.post("/editar-perfil", upload.single("foto"), async (req, res) => {
  ...
  if (!id || !nome || !email || !matricula) {
    return res
      .status(400)
      .json({ Mensagem: "Todos os campos obrigatórios devem ser preenchidos!" });
  }
  ...
  try {
    ...
    if (req.file) {
      ...
      const newPath = path.join(__dirname, "uploads/fotos", newFilename);
      
      // 👇 LIMPA TODAS AS EXTENSÕES ANTIGAS ANTES DE SALVAR A NOVA
      const fotosDir = path.join(__dirname, "uploads/fotos");
      const extensoesPossiveis = ['.jpg', '.jpeg', '.png', '.gif', '.webp'];
    ...
    let params = [nome, email, matricula];
